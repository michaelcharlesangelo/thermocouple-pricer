...
const TEAM_AUTH_STORAGE_KEY = "thermocouple_team_authed";
const TEAM_NAME_STORAGE_KEY = "thermocouple_team_name";
const TEAM_ACTIVITY_STORAGE_KEY = "thermocouple_team_last_activity";
...
export default function Home() {
  ...
  async function loadRatesAndConfig() {
    const [r, c, s] = await Promise.all([
      ...
      fetch("/api/stock/summary", { cache: "no-store" }).then((res) => res.json()),
    ]);
    setRates(r);
    setConfig(c);
  ...
  // Check for an existing session on load, and enforce the 15-minute idle
  // timeout - if too long has passed since the last recorded activity
  // (tracked in localStorage so it survives reloads), log out immediately.
  useEffect(() => {
    if (typeof window === "undefined") return;
    const wasAuthed = localStorage.getItem(TEAM_AUTH_STORAGE_KEY) === "true";
    const lastActivity = Number(localStorage.getItem(TEAM_ACTIVITY_STORAGE_KEY) || 0);
    if (wasAuthed && Date.now() - lastActivity < IDLE_TIMEOUT_MS) {
      setAuthed(true);
    } else {
      localStorage.removeItem(TEAM_AUTH_STORAGE_KEY);
  ...
  function recordActivity() {
    localStorage.setItem(TEAM_ACTIVITY_STORAGE_KEY, String(Date.now()));
  }
  ...
  async function submitLogin() {
    ...
    try {
      ...
      localStorage.setItem(TEAM_AUTH_STORAGE_KEY, "true");
      localStorage.setItem(TEAM_NAME_STORAGE_KEY, nameInput.trim());
      recordActivity();
      setAuthed(true);
      setPasswordInput("");
    } catch (e) {
  ...
  // While logged in: track activity (so idle time resets on real use) and
  // periodically check whether 15 minutes have passed since the last one.
  useEffect(() => {
    if (!authed) return;
    recordActivity();
      ...
      <>
        ...
        <div className="card" style={{ maxWidth: 360, margin: "40px auto 0" }}>
          ...
          <button
            ...
            onClick={submitLogin}
            disabled={authenticating || !nameInput.trim() || !passwordInput}
          >
      ...
      <div className="card">
        <div className="rate-stats">
          <div className="rate-stat">
            <span className="rate-stat-label">Platinum</span>
            <span className="rate-stat-value">${rates?.platinumUsdPerOz ?? "-"}/oz</span>
          </div>
          <div className="rate-stat">
            <span className="rate-stat-label">Rhodium</span>
            <span className="rate-stat-value">${rates?.rhodiumUsdPerOz ?? "-"}/oz</span>
          </div>
          <div className="rate-stat">
            <span className="rate-stat-label">USD/EUR</span>
            <span className="rate-stat-value">{rates?.usdEurRate ?? "-"}</span>
          </div>
          <div className="rate-stat">
            <span className="rate-stat-label">USD/IDR</span>
            <span className="rate-stat-value">{rates ? Math.round(rates.usdIdrRate) : "-"}</span>
          </div>
        </div>

        <div className="rate-footer">
          <p className="subtle rate-meta" style={{ margin: 0 }}>
            {rates && (
              <>
                Metal admin-set, updated {new Date(rates.metalUpdatedAt).toLocaleString()}
                <br />
                FX updated {new Date(rates.fxUpdatedAt).toLocaleString()}
              </>
            )}
          </p>
      ...
      {breakdown && (
        <div className="card">
          <div className="price-display">
            ...
            <div className="subtle" style={{ marginTop: 2, fontSize: "0.78rem", textTransform: "uppercase", letterSpacing: "0.02em" }}>
              {type} {diameter.toFixed(2)}, {configuration === "duplex" ? "Duplex" : "Simplex"}, LBH {lengthMm}mm
            </div>
            ...
            <tbody>
              {stockSummary.map((s) => (
                <tr key={`${s.type}-${s.diameterMm.toFixed(2)}`}>
                  ...
                  <td>{s.totalRemainingM.toLocaleString("id-ID", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} m</td>
                </tr>
              ))}
            </tbody>
          </table>
