              ...
              {(stock?.rolls ?? [])
                ...
                .map((r) => (
                  <option key={r.id} value={r.id}>
                    {r.type} {r.diameterMm.toFixed(2)} — {r.id} ({r.remainingLengthM.toFixed(1)}m left)
                  </option>
                ))}
            </select>
          </div>
              ...
              {(stock?.rolls ?? []).map((r) => (
                <Fragment key={r.id}>
                  <tr>
                    ...
                    <td>{r.id}</td>
                    <td>{r.totalLengthM.toFixed(1)}</td>
                    <td>{r.remainingLengthM.toFixed(1)}</td>
