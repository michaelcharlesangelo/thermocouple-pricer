...
## New: the calculator/stock pages are now also password protected

Separate from the admin password (Option B from our discussion) - so sales
can view prices without having access to change any rates or config.

- **Starting password**: `tempsens` (change it immediately from `/admin` →
  Security tab → "Team password")
- Sales enters their **name** (anything - it's just for the login log, not
  a real account) plus the shared password
- **Auto-logout after 15 minutes of inactivity** - applies to both this and
  the admin login. Resets on any click/keypress/scroll, so it won't log
  someone out mid-use, only after genuine idle time.
- **Login history**: `/admin` → Security tab shows the last 1000 logins
  (name + timestamp), 10 per page, with Prev/Next pagination. Auto-capped
  at 1000 entries so it never needs manual cleanup just to stay a
  reasonable size. There's also a "Clear all" button - worth treating as a
  rare, deliberate action rather than routine cleanup, since it also erases
  the audit trail itself (which somewhat defeats the point of having a log
  in the first place - the auto-cap exists so you shouldn't need to reach
  for this often, if ever).

**Worth knowing**: this stops someone from casually opening the link and
seeing prices - but it doesn't stop a screenshot. If a salesperson pastes a
number into a chat with a customer, that number is out regardless of how
locked-down the page itself is. This protects the tool, not any specific
figure once someone's chosen to share it.
